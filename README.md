...
📋 Architectural Flow
  ...
  1. User Input: The user asks a question in plain English/Turkish.
  
  2. RAG Retrieval: The system fetches relevant business rules from the PDF via similarity search.
  
  3. Agent Planning: Llama 3 formulates a step-by-step plan to solve the query using the available data tools.
  
  4. Action & Observation: The agent executes Python code in a secure REPL environment and observes the output.
  
  5. Synthesis: The final result is synthesized from the numerical data and document-based rules into a human-readable strategic recommendation.
